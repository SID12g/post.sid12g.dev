...
date: "2024-05-07"
description: Next 13에서 mdx 사용하기
image: /posts/Next-mdx/Thumbnail.png
tag: Next
---

오늘은 Next.js에서 mdx를 사용하는 법을 작성해보겠습니다.

지금 이 블로그도 mdx를 사용하여 제작되었습니다. 많은 분들에게 도움이 되면 좋겠습니다. 😄
...
그리고 contents 파일을 만들어서 test.mdx 라는 테스트 파일을 하나 만들어 줍니다.

![파일 디렉토리](/posts/Next-mdx/directory.png)

그 후 test.mdx에 아래와 같은 내용을 넣어주겠습니다.
...
코드 같은 경우 (```)로 코드를 감싸고 첫줄에 언어를 작성해줘야 합니다.
...
코드를 설명하자면, gray-matter를 통해 mdx 파일의 작성일자, 태그, 제목, 설명 등을 쉽게 가져와서 메타데이터를 만들고, 원하는 곳에 위치시킬 수 있습니다.

그리고 fs를 이용하여 시스템에서 mdx 파일을 읽고, path를 이용해 mdx 파일의 경로를 찾아 mdx 파일의 파일명을 사용하여 params를 결정합니다.

가져온 mdx 파일을 MDXRemote 컴포넌트를 이용하여 나타냅니다.

MDXRemote에 대한 스타일링은 겉에 div를 둘러 해당 div를 스타일링하여 대신할 수 있습니다.

![content](/posts/Next-mdx/first-mdx.png)

위와 같은 페이지가 나타난다면 성공입니다!

## 모든 mdx 파일을 가져오기 위한 함수 만들기

메인 페이지에 모든 mdx 파일들의 리스트를 가져오기 위해서 해당 기능을 위한 함수를 만들어보겠습니다.
...
위에 코드 또한 fs, path, matter를 통해 모든 mdx 파일의 정보를 가져온 후 아래와 같은 리스트를 반환합니다.
...
test2.mdx, test3.mdx라는 mdx 파일 2개를 더 만들어주겠습니다. 그리고 export된 contents를 page.tsx에 불러와서 contents안에 있는 요소 수 만큼 해당 content로 이동할 수 있는 미리보기를 만들어주겠습니다.
...
![contents 리스트](/posts/Next-mdx/mdx-list.png)

(저는 test3.mdx의 태그를 test.mdx와 같이 태그 1로 해주었습니다.)

위 처럼 contents 폴더 내에 있는 mdx 파일들이 모두 있다면 성공입니다.

mdx 파일을 불러오는 내용은 다 끝났지만 이대로 끝내긴 아쉬우니 팁을 드리도록 하겠습니다! 😻

## h 태그에 링크 걸기

![title-link](/posts/Next-mdx/title-link.png)
...
![codeblock-style-last](/posts/Next-mdx/code-last.png)
...
![codeblock-style-first](/posts/Next-mdx/code-first.png)
...
![contents-list-tag](/posts/Next-mdx/mdx-list-tag.png)

저는 태그 별로 contents를 불러오는 기능을 추가해 보기도 했습니다. 해당 활용법이 궁금하시다면 [Github](https://github.com/SID12g/mdx-content/blob/main/src/app/contents/%5Bslug%5D/page.module.css) 를 참고해주세요.
